...
# interactive map
mapview::mapview(grid_poa, zcol = 'population')
...
This quick approach to calculate accessibility involves only 2 steps. The first step is to build the multimodal transport network using the `r5r::build_network()` function.
...
As you can see, we only need to pass the path to our data directory to the `r5r::build_network()` function. The function then combines the OSM, GTFS and elevation data in this directory to create a graph that is used for routing trips between origin-destination pairs and, consequently, for calculating travel time matrices and accessibility.
