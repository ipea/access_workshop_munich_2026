...
First, we build the multimodal transport network by passing the path of our data directory to the `r5r::build_network()` function.
...
```{r}
# logistic decay
access_lgst <- accessibility::gravity(
  travel_matrix = ttm,
  land_use_data = points,
...
access_nexp <- accessibility::gravity(
  travel_matrix = ttm,
  land_use_data = points,
...
#| code-show: false
#| fig-height: 3
negative_exp <- accessibility::decay_exponential(decay_value = 0.1)
logistic <- accessibility::decay_logistic(cutoff = 15, sd = 5)
